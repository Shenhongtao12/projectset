...
public interface ReserveRepository extends JpaRepository<Reserve, Integer>, JpaSpecificationExecutor<Reserve> {
     ...
     * @return
     */
    @Query(value = "select id from reserve where (start_date_time>=?1 and end_date_time<=?1) OR (start_date_time>=?2 and end_date_time<=?2) OR (start_date_time>=?1 and end_date_time<=?2) and laboratory_id=?3 and status=?4 limit 1" ,nativeQuery = true)
    Integer existsReserve(LocalDateTime startDateTime, LocalDateTime endDateTime, Integer laboratoryId, Integer s);

    @Query(value = "select status from reserve where (start_date_time>=?1 and end_date_time<=?1) OR (start_date_time>=?2 and end_date_time<=?2) OR (start_date_time>=?1 and end_date_time<=?2) and laboratory_id=?3 and user_id=?4 limit 1" ,nativeQuery = true)
